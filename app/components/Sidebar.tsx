...
export default function Sidebar() {
  ...
  return (
    ...
    >
      ...
      <div className="w-8 h-8 rounded-full bg-blue-500 mb-6 overflow-hidden flex items-center justify-center">
        <div className="text-white text-sm">K</div>
      </div>
      
      {/* Navigation icons */}
      <nav className="flex flex-col items-center gap-3 flex-1 w-full">
        {/* Dashboard icon */}
        <Link href="/dashboard" className={`flex items-center justify-center text-white hover:bg-blue-700 transition-all duration-200 ${isExpanded ? 'w-full justify-start px-3' : 'w-full h-10'} ${!isExpanded && 'bg-blue-600'}`}>
          <div className={`flex items-center ${isExpanded ? 'h-10' : 'h-10 w-full flex justify-center'}`}>
        ...
        {/* Create icon */}
        <Link href="#" className={`flex items-center justify-center text-gray-400 hover:text-white hover:bg-blue-700 transition-all duration-200 ${isExpanded ? 'w-full justify-start px-3' : 'w-full h-10'}`}>
          <div className={`flex items-center ${isExpanded ? 'h-10' : 'h-10 w-full flex justify-center'}`}>
            <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
